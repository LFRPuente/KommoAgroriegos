...
const noPagadoStageId = Number((CFG.statusIds || {}).no_pagado_stage || 0);
...
for (const lead of canonicalLeads.values()) {
  ...
  const isInNoPagadoStage = noPagadoStageId > 0 && currentStatusId === noPagadoStageId;
  ...
  if (statusPagoDecision === "pagado") {
    ...
    reminderType = "REVIEW_TO_NO_PAGADO";
    stageTarget = noPagadoStageId > 0 ? noPagadoStageId : currentStatusId;
